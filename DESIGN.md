...
| Footer description | 开源硬件 重新定义个人机器人 PR X1 | Open Hardware for Embodied Robotics |

当前价格卡产品必须保持三档，OpenArm Upper Body / OpenArm 上半身不在官网价格区展示：

| 产品 | 中文定位 | 当前价格展示 |
| --- | --- | --- |
| M-OpenArm Edu | 教育基础版，适合高校教学与极客入门。 | `¥29,800起` / `$4,100 and up` |
| M-OpenArm Pro | 专业科研版，适合算法团队与进阶研究。 | `¥99,000起` |
| M-OpenArm Max | 至尊具身版，适合全向移动与长序列全场景操作任务。 | `¥129,000起` |
...
| Token | 色值 | 用途 |
| --- | --- | --- |
| `--color-canvas` | `#F2F2F2` | 页面主背景，浅灰。 |
| `--color-canvas-2` | `#E7E7E7` | 交替区块、浅色过渡背景、页脚分层。 |
| `--color-surface` | `#FFFFFF` | 普通卡片、价格卡、弹窗底色。 |
| `--color-surface-2` | `#DEDEDE` | 抬升层、媒体托盘、浅色区块分层。 |
...
页面主体背景必须保持浅灰 `#F2F2F2`，并在相邻区块之间使用 `--color-canvas` / `--color-canvas-2` 交替：

```css
radial-gradient(ellipse at 78% -12%, rgba(8, 127, 121, 0.04), transparent 34rem),
...
- 主视频：`public/videos/65691b4a635d38d33710943577e84dec.mp4`。
- Poster：`public/videos/65691b4a635d38d33710943577e84dec.jpg`。
...
- 当前媒体结构：视频必须作为全屏首屏背景铺满 viewport，不使用画中画、内嵌视频框或模糊 poster 边缘填充。
- 桌面前景视频：`object-fit: cover`，`object-position: 50% center`，`transform: translateY(3.25rem) scale(1.1)`。
- 手机前景视频：`object-fit: cover`，`object-position: 50% center`，`transform: translateY(3rem) scale(1.1)`。
- 视频色调：`brightness(1.16) contrast(1.02) saturate(1.04)`。
- 首屏媒体 framing 必须优先保持全屏沉浸感；如果出现机器人头部接近导航，使用全屏安全下移，不得把视频改回内嵌框。
...
```css
linear-gradient(90deg, rgba(245, 242, 236, 0.18) 0%, rgba(245, 242, 236, 0.12) 34%, rgba(245, 242, 236, 0.04) 62%, transparent 100%),
linear-gradient(90deg, rgba(7, 8, 8, 0.42) 0%, rgba(7, 8, 8, 0.30) 30%, rgba(7, 8, 8, 0.10) 60%, rgba(7, 8, 8, 0) 100%),
radial-gradient(ellipse at 27% 52%, rgba(7, 8, 8, 0.16), transparent 40%),
linear-gradient(180deg, rgba(7, 8, 8, 0) 0%, rgba(7, 8, 8, 0) 44%, rgba(7, 8, 8, 0.06) 100%)
```

底部过渡：

```css
linear-gradient(180deg, rgba(7, 8, 8, 0) 0%, rgba(7, 8, 8, 0.10) 40%, rgba(242, 242, 242, 0.86) 74%, #f2f2f2 88%, #f2f2f2 100%)
```

氛围光：

```css
linear-gradient(100deg, rgba(244, 240, 232, 0.03) 0%, transparent 31%),
linear-gradient(78deg, transparent 0%, transparent 50%, rgba(65, 214, 204, 0.12) 74%, transparent 100%),
linear-gradient(180deg, rgba(142, 255, 240, 0.035) 0%, transparent 32%, rgba(7, 8, 8, 0.10) 100%)
```
...
- H1 颜色：`#f4f0e8`。
- 副标题颜色：`#eee6d8`，使用 `drop-shadow(0 2px 14px rgba(0,0,0,0.46))` 保证视频浅色区域上的可读性。
...
- 位置：`fixed`，顶部 `16px`。
- 高度：手机 `56px`，桌面 `60px`。
- 顶部偏移：`8px`，确保首屏机器人头部不被导航遮挡，同时保留全屏视频沉浸感。
- 外侧横向边距：手机 `12px`，桌面 `24px`。
- 形状：pill。
- 初始背景：`#f3eee5` 72% 透明度，`backdrop-blur-xl`，边框 `#d9d0c2` 70%。
- 滚动后背景：`#f3eee5` 88% 透明度，`backdrop-blur-2xl`，边框 `#cfc6b8` 80%。
- 滚动后阴影：`0 20px 60px rgba(41,34,24,0.18)`。
- 链接默认：`#48524d`，字重 `500`。
- 链接 hover：`--color-cyan-400`。
- 品牌：`OpenArm × 智谷原子` 或 `OpenArm × NVatom`。
- 品牌 Badge：`border-cyan-400/35 bg-cyan-300/10 text-cyan-400`。
- 语言切换：圆角 `8px`，暖白底 `#f8f3ea`，边框 `#cfc6b8`，文字 `#171a18`，hover 使用 cyan。
...
```css
linear-gradient(135deg, #b8fff7 0%, #54ded4 48%, #0f9d93 100%)
```
...
inset 0 -1px 0 rgba(3, 68, 65, 0.26),
0 0 0 1px rgba(49, 200, 190, 0.22),
0 18px 42px rgba(17, 122, 115, 0.14)
```
...
- 价格使用 `text-cyan-400`。
- 中文价格后必须显示 `起`，例如 `¥29,800起`。
- 英文价格后必须显示 `and up`，例如 `$4,100 and up`；英文界面使用美元价格。
- 价格卡顶部版本标签使用小号等宽字、正常大小写和可换行标签组；`PRE-SALE` 胶囊必须 `shrink-0`，不得被 `Professional`、`Max Ultimate` 等长标签挤压。
- 机器人渲染图放在浅色托盘内，保持 `object-contain`。
- 如果源图留白过多，允许单张价格卡使用轻微 `scale` 放大，但不得裁掉机器人结构主体。
...
- `public/videos/65691b4a635d38d33710943577e84dec.mp4`：当前首屏 Hero 视频。
- `public/videos/research-box-demo.mp4`：箱子搬运、桌面放置。
...
- `public/images/wowrobo-openarm-2-94352134e53e4116971d617a8ddb56fa.png`：OpenArm 上半身价格卡。
