...
| 产品 | 中文定位 | 当前价格展示 |
| --- | --- | --- |
| OpenArm Upper Body / OpenArm 上半身 | 仅上半身结构，适合桌面教学、入门具身智能实验与低成本操作数据采集。 | `¥26,000起` / `$3,600 and up` |
| M-OpenArm Edu | 教育基础版，适合高校教学与极客入门。 | `¥69,000起` |
...
```css
linear-gradient(180deg, rgba(7, 8, 8, 0) 0%, rgba(7, 8, 8, 0.10) 40%, rgba(232, 227, 218, 0.86) 74%, #e8e3da 88%, #e8e3da 100%)
```
...
- 价格使用 `text-cyan-400`。
- 中文价格后必须显示 `起`，例如 `¥26,000起`。
- 英文价格后必须显示 `and up`，例如 `$9,600 and up`；不在未确认汇率时把人民币价格换算成美元。
