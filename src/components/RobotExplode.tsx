...
export default function RobotExplode({ activeStep = 0 }: RobotExplodeProps) {
  ...
  return (
    ...
    >
      ...
      >
        <hemisphereLight args={["#D8DCE8", "#1A1F2A", 0.7]} />
        <ambientLight intensity={0.45} color="#E8ECF0" />
        <directionalLight position={[4, 6, 5]} intensity={1.6} color="#FFFFFF" />
        <directionalLight position={[-5, 3, -3]} intensity={0.7} color="#A8C0E0" />
        <directionalLight position={[0, -4, 4]} intensity={0.35} color="#FFFFFF" />
        <directionalLight position={[6, 0, -5]} intensity={0.5} color="#FFE8C8" />
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at center, transparent 36%, rgba(8,10,14,0.58) 100%)",
        }}
      />
